...
- **R-050 (Workflows Operacionais Determinísticos — Pipelines de Estado Finito)**: Toda solicitação de desenvolvimento de software deve ser vinculada e executada rigorosamente sob um dos **8 Workflows Canônicos e de Ciclo de Vida**. Cada workflow constitui uma máquina de estados finita predefinida, onde a ordem dos estados, papéis dos agents e artefatos de transição são determinísticos:
  ...
  7. **`WORKFLOW-FRAMEWORK-MIGRATION` (Migração de Framework / Plataforma / Major Version)**:
     `[1. Pre-Flight Assessment: @tech-solution-architect] -> [2. Migration Phasing & Blueprint: @tech-solution-architect (com checkpoint humano)] -> [3. Codemod em Lote: specialist-developer (R-046)] -> [4. Paridade Funcional: specialist-unit-test] -> [5. Baseline Quality Gate: runtime-verifier + @code-review]`.
