...
- [ ] Se artefato possuir ferramentas mutativas (`insert_edit_into_file`, `create_file`): inclusão compulsória da regra de uso 100% obrigatório de context-mode (R-008 / R-056) para leituras e modificações/criações, e proibição estrita de ferramentas nativas de editor e terminal quando context-mode estiver disponível (rebaixadas a fallback exclusivo de indisponibilidade comprovada) no bloco CRÍTICO e inclusão de `.github/skills/efficient-batch-code-modification/SKILL.md` em `source_docs` (R-046 / R-051 / Smell 2.16 / Smell 2.24).
- [ ] Se artefato possuir ferramentas de sandbox (`context-mode/ctx_execute`): inclusão compulsória de `'context-mode/ctx_batch_execute'` em `tools:` e da regra de Single-Turn MCP Batching, vedando MCP Tool Chaining sequencial no chat (Smell 2.26).
