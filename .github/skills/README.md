...
| **`governance-audit-patterns`** | 🕵️ **Tier 1** | Catálogo de agent/governance smells (anti-padrão estrutural, gap de perfil, desalinhamento contratual perfil ↔ tools ↔ skills ↔ catálogo, gap de diretriz, diretriz redundante, gap de cobertura de categoria, vazamento de evidência real R-044) com sintoma, detecção, severidade e remediação — base do agent `agent-auditor` |
