...
- **Execução Obrigatória via Context Mode (R-008 — Think in Code)**: O uso de `context-mode` MCP (`ctx_execute`, `ctx_execute_file`, `ctx_batch_execute`, `ctx_index`, `ctx_search`) é 100% OBRIGATÓRIO para leituras, modificações e criações sempre que disponível no ambiente. Ferramentas nativas de editor e terminal de varredura são rebaixadas a fallback exclusivo de indisponibilidade comprovada (ver `CLAUDE.md` § R-008 e `.github/skills/context-mode/SKILL.md`).
...
- **Edições Agrupadas e em Lote (R-046 — Single-Turn Batching)**: Todas as alterações devem ser executadas compulsoriamente via script em processo único no sandbox (`ctx_execute`) com diffs cirúrgicos, ou em lote na mesma rodada de tools caso em fallback. Proibido encadeamento sequencial de chamadas no chat (ver `CLAUDE.md` § R-046 e `.github/skills/efficient-batch-code-modification/SKILL.md`).
...
- **Precedência Mandatória de Context Mode (R-056 — Anti-Editor Tool Sprawl)**: Ferramentas nativas de editor (`read_file`, `replace_string_in_file`, `create_file`, etc.) e comandos de terminal para inspeção são estritamente proibidos enquanto o context-mode estiver acessível. O processamento opera no sandbox para eliminar vazamento de bytes no chat (ver `CLAUDE.md` § R-056).
...
O uso de `context-mode` MCP é 100% OBRIGATÓRIO para leituras, modificações e criações de arquivos sempre que disponível no ambiente, eliminando vazamento de bytes e prevenindo truncamentos. As regras normativas de Think-in-Code, substituições mandatórias, bloqueios, Limiar >= 2 (Smell 2.26), Protocolo Plan-Then-Batch (etapa ENUMERAR -> CONSOLIDAR -> DESPACHAR; comandos curtos como "prosseguir" mantêm o rigor; Smell 2.13), Teto Rígido de Tool Turns <= 5, Warm Start e Edge Truncation / Destilação estão centralizadas em `CLAUDE.md` (§ R-008, § R-046, § R-056, § R-059, § R-060) e nas skills `.github/skills/context-mode/SKILL.md`, `.github/skills/efficient-batch-code-modification/SKILL.md` e `.github/skills/terminal-governance/SKILL.md`.
