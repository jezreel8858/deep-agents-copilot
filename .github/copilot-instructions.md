...
- **Edições agrupadas e em lote (`efficient-batch-code-modification` — R-046)**: todas as alterações de arquivos devem ser executadas preferencial e compulsoriamente via script em processo único no sandbox (`ctx_execute`), com diffs cirúrgicos mínimos e sem chamadas repetitivas de editor. Quando em fallback excepcional por indisponibilidade de context-mode, as alterações devem ser emitidas em lote na mesma rodada de tool calls (*single-turn batching*).
