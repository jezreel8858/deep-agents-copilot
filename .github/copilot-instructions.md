...
[Execução Sequencial no Workflow Canônico (R-050)]
    ├─ WORKFLOW-BUG-FIX: @bug-triage (RCA 2 fontes + flaky/regressão) → red-test (baseline) → bug-fixer (blast radius & rollback) → green-test & mini mutation → quality-gate & quality review loop (R-050.4 / canary)
    ├─ WORKFLOW-REFACTORING: rules (ground truth) → blast radius & contract testing (Pact) → @refactor-planner (Mikado DAG) → batch-exec → validation, redundância proporcional & quality review loop (R-050.4 / rollback blast_radius_revertido)
    ├─ WORKFLOW-TECHNICAL-ANALYSIS: scope → deterministic analysis → report
    ├─ WORKFLOW-FEATURE-DEVELOPMENT: requirements → blueprint → test-strategy → TDD → duplo gate & quality review loop (R-050.4) → PR
    ├─ WORKFLOW-GOVERNANCE-MAINTENANCE: audit → human approval → batch execution → quality gate (Tier 1) & quality review loop (R-050.4)
    ├─ WORKFLOW-DEPENDENCY-VULNERABILITY-REMEDIATION: scan → blast-radius → bump → adapt → quality-gate & quality review loop (R-050.4)
    ├─ WORKFLOW-FRAMEWORK-MIGRATION: assessment (5D + symbol exhaustion) → blueprint & matriz de-para → codemod anti-omission → dual-verification parity → quality-gate baseline → post-migration redundancy gate & quality review loop (R-050.4)
